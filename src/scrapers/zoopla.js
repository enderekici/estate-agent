...
const { buildZooplaUrls } = require('./search-url-builders');

const SOURCE = 'zoopla';
const BASE_URLS = buildZooplaUrls();
const MAX_PAGES = 12;
    ...
    for (const baseUrl of BASE_URLS) {
      ...
      try {
        ...
        while (pageNum <= MAX_PAGES && stalePages < MAX_STALE_PAGES) {
          const url = pageNum === 1 ? baseUrl : `${baseUrl}&pn=${pageNum}`;
          ...
          const pageData = await page.evaluate(() => {
            ...
            const results = rows.map((row) => {
              const linkEl = row.querySelector('a[href*="/for-sale/details/"]:not([href*="/contact/"])')
                || row.querySelector('a[href*="/for-sale/details/"]');
              if (!linkEl) return null;
              const url = linkEl.href || linkEl.getAttribute('href') || '';
              if (!url || url.includes('/contact/') || seen.has(url)) return null;
              seen.add(url);
